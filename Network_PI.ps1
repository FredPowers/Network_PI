...
.NOTES
	NAME:	NetworkPI.ps1
    VERSION : 1.2  25/10/2022
	AUTHOR:	Frédéric Puren

Network Private Investigator
...
# Demande de lancement en administrateur
...
# Fin de la fonction Calcul_Reseau -------------------------------------------------------------------------------------------------------

# Fonction CIDRToMask

function CIDRToMask($MasqueCIDR)
{
...
# Fin de la fonction CIDR To Masque --------------------------------------------------------------------
...
Do{
    ...
    switch ($choix)
    {
        ...
        1{
            ...
            $CIDR = get-netipaddress | where {$_.interfaceAlias -eq $InterfaceUp -and $_.AddressFamily -eq "IPv4"} | select -ExpandProperty PrefixLength -ErrorAction SilentlyContinue
            ...
            # Appel fonction CIDRToMask
            $MaskIP = CIDRToMask $CIDR

            Write-Host "Adresse IP           : $IP_Host"
            write-host "Masque sous-réseaux  : $MaskIP / CIDR: $CIDR"
            ...
            Calcul_Reseaux $AdresseIPHost $MaskIP
        ...
        3{
            ...
            #interface physique active
            $InterfaceUp = (Get-NetAdapter -Physical | where {$_.status -like "Up"}).Name
            $CIDR = get-netipaddress | where {$_.interfaceAlias -eq $InterfaceUp -and $_.AddressFamily -eq "IPv4"} | select -ExpandProperty PrefixLength
        ...
        4{
            ...
            $CIDR_VPN = get-netipaddress | where {$_.interfaceAlias -eq $InterfaceUp_VPN -and $_.AddressFamily -eq "IPv4"} | select -ExpandProperty PrefixLength
            ...
            $MaskIP_VPN = CIDRToMask $CIDR_VPN
            Write-Host "Adresse IP           : $IP_VPN"
            write-host "Masque sous-réseaux  : $MaskIP_VPN / CIDR: $CIDR_VPN"
            Write-Host "DNS                  : $DNS_Host"
            ...
            Calcul_Reseaux $AdresseIP_VPN $MaskIP_VPN
            }
